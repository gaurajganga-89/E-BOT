...
void OnTick()
{
  ...
  if(PanelShouldUpdate(newBar))
  {
    ...
    int ln=0;
    PanelSetLine(ln++, pc, BOT_NAME+" v1.35");
  ...
  req.sl        = sl;
  req.tp        = tp;
  req.comment   = BOT_NAME+" "+best.name+" sc="+DoubleToString(best.score,2)+" v1.35";
